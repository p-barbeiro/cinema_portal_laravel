                ...
                <div class="table-cell">
                    @if ($seat['status'] == 'available')
                        <div class="cursor-pointer flex justify-center items-center text-sm m-0.5">
                            ...
                            <label for="seat-{{ $seat['label'] }}" class="dark:border-gray-800 border border-gray-400 cursor-pointer text-xs min-w-4 min-h-4 md:w-6 md:h-6 bg-green-200 hover:bg-orange-200 flex items-center justify-center rounded">
                                <div id="seat-label-{{ $seat['label'] }}" class="text-xs hidden text-orange-500">
                                    {{ $seat['label'] }}
                                </div>
                            </label>
                        </div>
                    ...
                    @elseif($seat['status'] == 'occupied')
                        <div class="cursor-pointer flex justify-center items-center text-sm m-0.5">
                            ...
                            <label for="seat-{{ $seat['label'] }}" class="dark:border-gray-800 border border-gray-400 bg-red-200 md:w-6 md:h-6 min-w-4 min-h-4 flex items-center justify-center rounded"></label>
                        </div>
                    @else
                        <div class="cursor-pointer flex justify-center items-center text-sm m-0.5">
                            <input type="checkbox" id="seat-{{ $seat['label'] }}" name="seats[]" value="{{ $seat['id'] }}" class="hidden" checked disabled>
                            <label for="seat-{{ $seat['label'] }}" class="dark:border-gray-800 border border-gray-400 bg-orange-300 md:w-6 md:h-6 min-w-4 min-h-4 flex items-center justify-center rounded">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        document.querySelectorAll('input[type="checkbox"]').forEach(function (checkbox) {
            checkbox.addEventListener('change', function () {
                const seatLabelId = 'seat-label-' + this.id.split('-')[1];
                const labelDiv = document.getElementById(seatLabelId);
                if (this.checked) {
