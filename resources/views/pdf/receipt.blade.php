...
<div id="container">
	<div id="header">
		<div id="logo">
			<img src="{{public_path()."/img/receipt_logo.png"}}" alt="" style="max-height: 100px">
		</div>
		<div id="reference">
	...
	<div id="items">
		<table>
			...
			@foreach($cart->sortby('id') as $ticket)
				@php
				...
				<tr style="vertical-align: middle;">
					<td>{{$ticket['movie']}} | {{$ticket['date']}} | {{date('H:i',strtotime($ticket['start_time']))}} | Seat: {{$ticket['row'].$ticket['seat_number']}} | Theater:{{$ticket['theater']}}</td>
					<td>{{$full_price}} €</td>
					<td>1</td>
	...
	<div id="summary">
		<div id="total">
			<table>
				<tr>
					<td>Total</td>
					<td>{{number_format($cart->sum('price'),2)}} €</td>
				</tr>
				<tr>
					<td>Discount</td>
					<td>{{auth()->check()?'-' . number_format($ticket_discount * $cart->count(),2) . '€' : 'None'}}</td>
				</tr>
				<tr>
					<td>Final Total</td>
